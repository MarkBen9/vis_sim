...
class HealPix:
    def __init__(self, s , Nside = 512):
        
        self.s_high_res = s
        self.Nside = Nside
        #--- Med
        m = ap.healpix.HealpixMap(nside=Nside/2)#Nside/2
        m.change_scheme('NEST')
        self.s_med_res = np.array(m.px2crd(np.arange(m.npix()))).T
        #--- Low
        l = ap.healpix.HealpixMap(nside=Nside/4)#Nside/4
        l.change_scheme('NEST')
        self.s_low_res = np.array(l.px2crd(np.arange(l.npix()))).T
       
    def Res_Map(A, I,  fqs, Err = 1.0e-6, Nside = 512, bls=14):   
...
class Source:
    ...
    def measurement_eq(self, antenna1 ,antenna2, lst, lat):
        ...
        return x
class measurment_eq_GPU:
    def __init__(self,A, I, s, ant_pos, fqs):
        '''A: (s,fq) 2-dim array
        I: (s,fq) 2-dim array
        ant_pos: (antenna's position,xyz)
        s: (sky position,xyz)
...
#selection of points of interest
        s_valid = s[:,2] > 0
        self.s_above = s[s_valid,:]
        self.A_above = A[s_valid,:]
        self.I_above = I[s_valid,:]
#dot product
        a_s = np.dot(ant_pos, self.s_above.T).T         
#Matrix set up
        a_s.shape = (a_s.shape[0],1,a_s.shape[1]) # shape is (s,fq,ant_pos)
        fqs.shape = (1,fqs.size,1)
        if self.A_above.ndim < 3:  
            self.A_above.shape = self.A_above.shape + (1,)
        if self.I_above.ndim < 3:
            self.I_above.shape = self.I_above.shape + (1,)
#Equation for just an antenna
        V_a = (np.sqrt(self.A_above * self.I_above) * np.exp(-2j*np.pi / C * fqs * a_s))
